...
#if defined(NDEBUG)
# error "AdamsCoinOfFun cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x52575597c23e68adef53811c27824873d74090d4db8e6cfedccdd9700f5f9b4c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // AdamsCoinOfFun: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "AdamsCoinOfFun Signed Message:\n";
...
{
    // AdamsCoinOfFun: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // AdamsCoinOfFun: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // AdamsCoinOfFun: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // AdamsCoinOfFun: 30 sec
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // AdamsCoinOfFun: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // AdamsCoinOfFun: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x52575597c23e68adef53811c27824873d74090d4db8e6cfedccdd9700f5f9b4c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Donald Trump is not Hitler. But there are eerie parallels";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("131384710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1518076671;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 4274195;
        ...
        assert(block.hashMerkleRoot == uint256("0xc2c3d61af4adcb4b9987e263c4d7fbdf2c5882171d94a49c860f4a5c7a84df5b"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // AdamsCoinOfFun: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AdamsCoinOfFunMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AdamsCoinOfFunMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AdamsCoinOfFunMiner(CWallet *pwallet)
{
    printf("AdamsCoinOfFunMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("adamscoinoffun-miner");
    ...
    try { loop {
        ...
        printf("Running AdamsCoinOfFunMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AdamsCoinOfFunMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AdamsCoinOfFunMiner, pwallet));
}
