...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "AdamsCoinOfFun " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
	{"vps170017.vps.ovh.ca", "158.69.60.76"},
	{NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. AdamsCoinOfFun is probably already running."), addrBind.ToString().c_str());
        else
